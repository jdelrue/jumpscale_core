...
class SystemProcess(object):
    ...
    def execute(self, command, showout=True, useShell=True, log=True, cwd=None, timeout=600, errors=[], ok=[], captureout=True, die=True, async_=False, env=None):
        ...
        command = self._j.data.text.strip(command)
        if "\n" in command:
            path = self._j.sal.fs.getTmpFilePath()
            self.logger.debug("execbash:\n'''%s\n%s'''\n" % (path, command))
            if die:
                command = "set -ex\n%s" % command
            self._j.sal.fs.writeFile(path, command + "\n")
            command = "bash %s" % path
        else:
        ...
        if rc<0 or rc>0:
            self._j.sal.process.logger.debug('system.process.run ended, exitcode was %d' % rc)
        if out!="":
            self._j.sal.process.logger.debug('system.process.run stdout:\n%s' % out)
        if err!="":
            self._j.sal.process.logger.debug('system.process.run stderr:\n%s' % err)
        ...
        if die and rc!=0:
            ...
            if command.find("\n") ==-1 and len(command)<40:
                ...
                command="\n".join(command.split(";"))
                msg+= self._j.data.text.indent(command).rstrip()+"\n\n"
            if out.strip()!="":
                msg+="stdout:\n"
                msg+= self._j.data.text.indent(out).rstrip()+"\n\n"
            if err.strip()!="":
                msg+="stderr:\n"
                msg+= self._j.data.text.indent(err).rstrip()+"\n\n"
    ...
    def executeAsyncIO(self, command, outMethod="print", errMethod="print", timeout=600,
        ...
        if cwd is not None:
            if not useShell:
                raise self._j.exceptions.Input(message="when using cwd, useshell needs to be used",
    ...
    #     cmd2 = self._j.dirs.replace_txt_dir_vars(cmd2)
    #     print(cmd2)
    #     self._j.sal.process.executeWithoutPipe(cmd2)
    ...
    def executeScript(self, scriptName):
        ...
        try:
            script = self._j.sal.fs.readFile(scriptName)
    ...
    def executeBashScript(
        ...
        self._j.sal.fs.writeFile(tmppathdest, content)

        if remote is not None:
            if sshkey:
                if not self._j.clients.ssh.sshkey_path_get(sshkey, die=False) is None:
            ...
            self._j.sal.fs.remove(tmppathdest)
        return rc, res, err
    ...
    def executeCode(self, code, params=None):
        ...
        use params=self._j.data.params.get() as input
        """
        if params is None:
            params = self._j.data.params.get()
        ...
        if len(self._j.data.regex.findAll("^def", code)) != 1:
            server.raiseError(
    ...
    def isPidAlive(self, pid):
        ...
        if self.isUnix:
            ...
            return self._j.sal.windows.isPidAlive(pid)
    ...
    def kill(self, pid, sig=None):
        ...
        """
        pid=int(pid)
        self._j.sal.process.logger.debug('Killing process %d' % pid)
        if self.isUnix:
            try:
                if sig is None:
                    sig = signal.SIGKILL
    ...
    def getPidsByFilterSortable(self, filterstr, sortkey=None):
        ...
        rcode, out, err = self._j.sal.process.execute(cmd)
        # print out
        found = []
        for line in out.split("\n"):
    ...
    def getProcessPid(self, process):
        if process is None:
            raise self._j.exceptions.RuntimeError("process cannot be None")
        if self.isUnix:
    ...
    def checkProcessRunning(self, process, min=1):
        ...
        # Windows platform
            ...
            return self._j.sal.windows.checkProcess(process, min)
    ...
    def checkProcessForPid(self, pid, process):
        ...
        if self.isUnix:
            ...
            i = 0
            for line in output.splitlines():
                if self._j.core.platformtype.myplatform.isLinux or self._j.core.platformtype.myplatform.isESX():
                    match = re.match(
            ...
            return self._j.sal.windows.checkProcessForPid(process, pid)
    ...
    def setEnvironmentVariable(self, varnames, varvalues):
        ...
        try:
            ...
            raise self._j.exceptions.RuntimeError(e)
    ...
    def getPidsByPort(self, port):
        ...
        pids = self._j.sal.process.getProcessPid(name)
        # print pids
        return pids
            ...
            for process in psutil.process_iter():
                try:
                    ...
                    if str(e).find("psutil.AccessDenied") == -1:
                        raise self._j.exceptions.RuntimeError(str(e))
                    continue
                if cc != []:
                    for conn in cc:
                        ...
                        if port == portfound:
                            return process
            return None
            # raise self._j.exceptions.RuntimeError("This platform is not supported in self._j.sal.process.getProcessByPort()")
    ...
    def getDefunctProcesses(self):
        rc, out, err = self._j.sal.process.execute("ps ax")
        llist = []
        for line in out.split("\n"):
            if line.strip() == "":
                continue
    ...
    def getEnviron(self, pid):
        environ = self._j.sal.fs.fileGetContents('/proc/%s/environ' % pid)
        env = dict()
    ...
    # DO NOT REENABLE, if you need it, call the self._j.sal.process.executeASyncIO
    ...
    #             useShell = False
    #         else:
    #             raise self._j.exceptions.RuntimeError("Platform not supported")
    #
