...
class RegexTools(JSBASE):
    ...
    def matchContent(
        ...
        if self.matchMultiple(
                ...
                text=content) and not self.matchMultiple(
    ...
    def matchPath(path, regexIncludes=[], regexExcludes=[]):
        if self.matchMultiple(
                patterns=regexIncludes,
                text=path) and not self.matchMultiple(
                patterns=regexExcludes,
                text=path):
            return True
        return False
    ...
    def extractBlocks(
        ...
        if type(blockStartPatterns).__name__ != 'list' or type(
                blockStartPatternsNegative).__name__ != 'list' or type(
                blockStopPatterns).__name__ != 'list' or type(
                blockStopPatternsNegative).__name__ != 'list' or type(
...
if __name__ == '__main__':
    ...
    regexmatches = rt.getRegexMatches(
