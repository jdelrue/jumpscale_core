...
class YAML(String):
    ...
    def fromString(self, s):
        ...
        """
        if self._j.data.types.dict.check(s):
            return s
        else:
            # s = s.replace("''", '"')
            self._j.data.serializers.yaml.loads(s)
            return s

    def toString(self, v):
        return self._j.data.serializers.yaml.dumps(v)
...
class Dictionary():
    ...
    def fromString(self, s):
        ...
        if self._j.data.types.dict.check(s):
            ...
            s = s.replace("''", '"')
            self._j.data.serializers.json.loads(s)
            return s

    def toString(self, v):
        return self._j.data.serializers.json.dumps(v, True, True)
...
class List():
    ...
    def list_check_1type(self, llist, die=True):
        if len(llist) == 0:
            return True
        ttype = self._j.data.types.type_detect(llist[0])
        for item in llist:
    ...
    def fromString(self, v, ttype=None):
        ...
        if ttype is not None:
            ttype = ttype.NAME
        v = self._j.data.text.getList(v, ttype)
        v = self.clean(v)
        if self.check(v):
            return v
        else:
    ...
    def clean(self, val, toml=False, sort=False, ttype=None):
        ...
        if len(val) == 0:
            return val
        if ttype is None:
            self.SUBTYPE = self._j.data.types.type_detect(val[0])
            ttype = self.SUBTYPE
        res = []
        for item in val:
            if not toml:
    ...
    def toml_value_get(self, val, key=""):
        ...
        if key == "":
            ...
            return self._j.data.serializers.toml.loads(val)
    ...
    def capnp_schema_get(self, name, nr):
        ...
        if self.SUBTYPE.BASETYPE in ["string", "integer", "float", "bool"]:
            ...
            # be stored
            capnptype = self._j.data.types.bytes.capnp_schema_get(
...
class Hash(List):
    ...
    def clean(self, value):
        ...
        def bytesToInt(val):
            if self._j.data.types.bytes.check(val):
                if len(val) is not 4:
                    raise RuntimeError(
        ...
        if self._j.data.types.list.check(value) or self._j.data.types.set.check(value):
