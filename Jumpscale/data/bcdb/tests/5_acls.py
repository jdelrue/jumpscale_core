...
def main(self):
    ...
    def do(bcdb):
        ...
        assert len(a.model.index.select()) == 0 #there should be no acl's in the DB yet
        ...
        assert len(bcdb.acl.index.select()) #means we have indeed the index for acl == 2
    ...
    bcdb = j.data.bcdb.new(name="test",zdbclient=zdbclient,cache=False)
    do(bcdb)
    bcdb.reset()
