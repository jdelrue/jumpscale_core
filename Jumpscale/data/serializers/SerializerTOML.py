...
class SerializerTOML(SerializerBase):
    ...
    def fancydumps(self, obj, secure=False):
        ...
        if not self._j.data.types.dict.check(obj):
            raise self._j.exceptions.Input("need to be dict")
        ...
        for key in keys:
            ...
            ttype = self._j.data.types.type_detect(val)
            if secure and key.endswith("_") and ttype.BASETYPE == "string":
                val = self._j.data.nacl.default.encryptSymmetric(
        ...
        return self._j.data.text.strip(out)
    ...
    def loads(self, s, secure=False):
        ...
        try:
            ...
            raise RuntimeError(
                ...
                (self._j.data.text.indent(s), str(e)))
        if secure and self._j.data.types.dict.check(val):
            res = {}
            for key, item in val.items():
                if key.endswith("_"):
                    res[key] = self._j.data.nacl.default.decryptSymmetric(
    ...
    def merge(
        ...
        """
        if self._j.data.types.string.check(tomlsource):
            try:
        ...
        if self._j.data.types.string.check(tomlupdate):
            try:
        ...
        return self._j.data.serializers.dict.merge(
