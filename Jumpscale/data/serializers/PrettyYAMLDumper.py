...
class PrettyYAMLDumper(yaml.dumper.SafeDumper, JSBASE):
    ...
    def represent_undefined(dumper, data):
        if isinstance(
                ...
                data,
                '_asdict'):
            return dumper.represent_odict(
                data._asdict())  # assuming namedtuple
...
class UnsafePrettyYAMLDumper(PrettyYAMLDumper):
    ...
    def choose_scalar_style(self):
        ...
        if is_dict_key:
            ...
            if self.event.value.endswith(':'):
                self.event.style = "'"
        return super(
            UnsafePrettyYAMLDumper,
            self).choose_scalar_style() if self.event.style != 'plain' else (
            "'" if ' ' in self.event.value else None)
    ...
    def represent_stringish(dumper, data):
        ...
        # Having such binary stuff pretty much everywhere on unix (e.g. paths)
        # kinda sucks
...
class PrettyYaml(JSBASE):
    ...
    def dump_add_vspacing(self, buff, vspacing):
        ...
        for line in buff:
            ...
            if ':' in line:
                ...
                if len(vspacing) > level and len(result) != 1:
                    ...
                    result.insert(-1,
                                  vspace if not isinstance(vspace,
                                                           int) else '\n' * vspace)
    ...
    def dump(
        ...
        if vspacing is not None:
            self.dump_add_vspacing(buff, vspacing)
    ...
    def dumps(self, data, **dump_kws):
        return self.dump(data, dst=bytes, **dump_kws)
    ...
    # p, _p = pprint, print
    # print = pprint # pyaml.print() won't work without "from __future__
