...
class SerializersFactory:
    ...
    def fixType(self, val, default):
        ...
        if self._j.data.types.list.check(default):
            res = []
            if self._j.data.types.list.check(val):
                ...
                if "," in val:
                    ...
                    if val not in res:
                        res.append(val)
        elif self._j.data.types.bool.check(default):
            if str(val).lower() in ['true', "1", "y", "yes"]:
                res = True
            else:
                res = False
        elif self._j.data.types.int.check(default):
            res = int(val)
        elif self._j.data.types.float.check(default):
            res = int(val)
        else:
            res = str(val)
        return res
...
#             self._j.data.serializer.getSerializerType(k, self.key)
...
#             # print "dumps:%s"%key
#             val = self._j.data.serializer.types[key].dumps(val)
#         return val
...
#             # print "loads:%s"%key
#             data = self._j.data.serializer.types[key].loads(data)
#         return data
