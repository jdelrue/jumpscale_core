...
class NumTools:
    ...
    @property
    def currencies(self):
        return self._j.clients.currencylayer.cur2usd
    ...
    def interpolateList(self, tointerpolate, left=0, right=None, floatnr=None):
        ...
        for xpos in range(len(tointerpolate)):
            if not tointerpolate[xpos] is None \
                and not self._j.data.types.int.check(
                    tointerpolate[xpos]):
                isint = False
    ...
    def text2val(self, value, curcode="usd"):
        ...
        """
        d = self._j.data.types.numeric.str2bytes(value)
        return self._j.data.types.numeric.bytes2cur(d, curcode=curcode)
    ...
    def bitstring_set_bit(self, bits, pos=7):
        ...
        if not self._j.data.types.string.check(bits):
