...
class JSLoader():
    ...
    def logger(self):
        if self._logger is None:
            self._logger = self._j.logging.get("jsloader")
        return self._logger

    @property
    def autopip(self):
        return self._j.core.state.configGet("system")["autopip"] in \
            [True, "true", "1", 1]

    def _installDevelopmentEnv(self):
        cmd = "apt-get install python3-dev libssl-dev -y"
        self._j.sal.process.execute(cmd)
        self._j.sal.process.execute("pip3 install pudb")
    ...
    def plugins(self):
        ...
        if self._plugins is None:
            ...
            state = self._j.tools.executorLocal.state
    ...
    def gather_modules(self, startpath=None, depth=0, recursive=True,
        ...
        for name, _path in self.plugins.items():
            ...
            if not self._j.sal.fs.exists(_path, followlinks=True):
                raise RuntimeError("Could not find plugin dir:%s" % _path)
            if not self._j.sal.fs.exists(path, followlinks=True):
                continue
        ...
        for jlocationRoot, jlocationRootDict in moduleList.items():
            # is per item under j e.g. self._j.clients
    ...
    def find_jsmodule(self, modulepath):
        ...
        for k, info in self._j.__jsmodlookup__.items():
    ...
    def reset_jsonmodules(self):
        self._plugins = None
        self._j.__jsmodlookup__ = {} # table that turns module to path
        self._j.__jsmodbase__ = {}
    ...
    def manage_jsonmodules(self, plugin, modbase):
        ...
        for jlocationRoot, jlocationRootDict in modlist.items():
            ...
            for subname, sublist in jlocationRootDict.items():
                ...
                self._j.__jsmodlookup__[pmodname] = info
    ...
    def load_json(self):
        ...
        for plugin, outJSON in self.jsonfiles.items():
            ...
            try:
                outJSON = self._j.sal.fs.fileGetContents(outJSON)
    ...
    def generate_json(self, pluginsearch=None):
        """ generates the jumpscale json file(s), walking the plugin directories
            ...
            stores the results in self._j.__jsjson__
        ...
        for plugin, outJSON in self.jsonfiles.items():
            ...
            self._j.sal.fs.writeFile(outJSON, modlistout_json)
            self.manage_jsonmodules(plugin, modbase)

    def _pip_installed(self):
        """ return the list of all installed pip packages
        """
        _, out, _ = self._j.sal.process.execute(
    ...
    def find_jslocations(self, path):
        ...
        """
        res = {}
        C = self._j.sal.fs.fileGetContents(path)
        classname = None
        locfound = False
        for line in C.split("\n"):
    ...
    def find_modules(self, path, moduleList=None, baseList=None, depth=None,
        ...
        initfiles = self._j.sal.fs.listFilesInDir(
            path, recursive, "__init__.py", depth=depth + 1)
        pyfiles = self._j.sal.fs.listFilesInDir(path, recursive, "*.py",
                                               depth=depth)
        pyfiles = initfiles + pyfiles
        for classfile in pyfiles:
            #print("found", classfile)
            basename = self._j.sal.fs.getBaseName(classfile)
    ...
    def removeEggs(self):
        for key, path in self._j.clients.git.getGitReposListLocal(
                account="jumpscale").items():
            for item in [item for item in self._j.sal.fs.listDirsInDir(
                    path) if item.find("egg-info") != -1]:
                self._j.sal.fs.removeDirTree(item)
