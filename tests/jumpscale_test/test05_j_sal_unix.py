...
class TestUNIX(TestcasesBase):
    ...
    def test001_check_application_installed(self):
        ...
        **Test Scenario:**
        ...
        """
        self.lg.info(" Check that nosetests app is installed, should return true. ")
        app_name = "nosetests"
        self.assertTrue(j.sal.unix.checkApplicationInstalled(app_name))

        self.lg.info(" Check that fake app is installed, should return false. ")
        app_name = "fake_name"                
    ...
    def test002_crypt(self):
        ...
        **Test Scenario:**
        ...
        """
        self.lg.info("Crypt word with 2 characters salt, should return crypted word started with salt characters .")
        salt = str(uuid.uuid4()).replace('-', '')[1:3]
        encrypted_word = j.sal.unix.crypt("test",salt)    
        self.assertIn(salt, encrypted_word)  

        self.lg.info("Crypt word with more than 2 characters salt, Should return crypted word started with first two chars of salt.")
    ...
    def test003_check_user_exist(self):
        ...
        **Test Scenario:**
        ...
        system_users = os.popen("cat /etc/passwd | cut -d : -f 1").read().splitlines()
    ...
    def test004_check_group_exist(self):
        ...
        **Test Scenario:**
        ...
        """
        self.lg.info("Check that one of system groups is exist, should succeed.")
        system_groups = os.popen("cat /etc/group | cut -d : -f 1").read().splitlines()
    ...
    def test05_create_new_group(self):
        ...
        **Test Scenario:**
        ...
        self.assertIn(group_name, os.popen("cat /etc/group | cut -d : -f 1").read().splitlines())
    ...
    def test07_change_file_owner(self):
        ...
        **Test Scenario:**
        ...
        self.lg.info("Check that the owner of [f1] updated to [U1], should succeed")
    ...
    def test08_change_folder_owner_recursively(self):
        ...
        **Test Scenario:**
        ...
        self.lg.info("Check that the owner of [D1] is updated to [U1], and [F1] owner isn't updated.")
        self.assertNotEqual(getpwuid(os.stat(file_path).st_uid).pw_name, user_name)
        self.assertEqual(getpwuid(os.stat(dir_path).st_uid).pw_name, user_name)

        self.lg.info("Change the [D1] owner to [U1] recursively, should succeed.")
        j.sal.unix.chown(dir_path , user_name, random_group, recursive =True)

        self.lg.info("Check that the owner of [D1] is updated to [U1], and [F1] owner too .")
        self.assertEqual(getpwuid(os.stat(file_path).st_uid).pw_name, user_name)
        self.assertEqual(getpwuid(os.stat(dir_path).st_uid).pw_name, user_name)        
