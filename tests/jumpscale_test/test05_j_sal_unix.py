...
class TestUNIX(TestcasesBase):
    ...
    def test001_check_application_installed(self):
        ...
        **Test Scenario:**
        ...
        self.lg.info(
            " Check that nosetests app is installed, should return true. ")
        app_name = "python3"
        self.assertTrue(self._j.sal.unix.checkApplicationInstalled(app_name))

        self.lg.info(
            " Check that fake app is installed, should return false. ")
        app_name = "fake_name"
        self.assertFalse(self._j.sal.unix.checkApplicationInstalled(app_name))
    ...
    def test002_crypt(self):
        ...
        **Test Scenario:**
        ...
        encrypted_word = self._j.sal.unix.crypt("test", salt)
    ...
    def test003_check_user_exist(self):
        ...
        **Test Scenario:**
        ...
        self.assertTrue(self._j.sal.unix.unixUserExists(random_user))

        self.lg.info("Check that fake user isn't exist, should succeed ")
        fake_user = self.random_string()
        self.assertFalse(self._j.sal.unix.unixUserExists(fake_user))
    ...
    def test004_check_group_exist(self):
        ...
        **Test Scenario:**
        ...
        self.assertTrue(self._j.sal.unix.unixGroupExists(random_group))
    ...
    def test05_create_new_group(self):
        ...
        **Test Scenario:**
        ...
        self._j.sal.unix.addSystemGroup(group_name)

        self.lg.info("Check that [G1] exist, should succeed.")
        self.assertTrue(self._j.sal.unix.unixGroupExists(group_name))
    ...
    def test06_add_user_to_group(self):
        ...
        **Test Scenario:**
        ...
        self._j.sal.unix.addSystemUser(user_name)
        ...
        system_groups = os.popen(
            "cat /etc/group | cut -d : -f 1").read().splitlines()
        random_group = random.choice(system_groups)
        self._j.sal.unix.addUserToGroup(user_name, random_group)

        self.lg.info("Check that [U1] exist on [G1], should succeed.")
        self.assertTrue(self._j.sal.unix.unixUserIsInGroup(user_name, random_group))

        self.lg.info(
            "Add fake_user to one of existing groups [G1], should fail.")
        fake_user_name = self.random_string()
        with self.assertRaises(AssertionError):
            self._j.sal.unix.addUserToGroup(fake_user_name, random_group)
    ...
    def test07_change_file_owner(self):
        ...
        **Test Scenario:**
        ...
        system_groups = os.popen(
            "cat /etc/group | cut -d : -f 1").read().splitlines()
        random_group = random.choice(system_groups)
        self._j.sal.unix.chown(file_path, user_name, random_group)
        ...
        with self.assertRaises(KeyError):
            self._j.sal.unix.chown(file_path, fake_user_name, random_group)

        self.lg.info(
            " Change the owner of non-exist file to [U1], should fail.")
        fake_file_path = "/{}".format(self.random_string())
        with self.assertRaises(FileNotFoundError):
            self._j.sal.unix.chown(fake_file_path, user_name, random_group)
    ...
    def test08_change_folder_owner_recursively(self):
        ...
        **Test Scenario:**
        ...
        system_groups = os.popen(
            "cat /etc/group | cut -d : -f 1").read().splitlines()
        random_group = random.choice(system_groups)
        self._j.sal.unix.chown(dir_path, user_name, random_group)
        ...
        self.lg.info(
            "Change the [D1] owner to [U1] recursively, should succeed.")
        self._j.sal.unix.chown(dir_path, user_name, random_group, recursive=True)
